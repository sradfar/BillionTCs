...
#
# This script visualizes joint exceedance probabilities of storm surge and precipitation 
# and their relationship to tropical cyclone economic damages. It creates copula-based 
# shaded subplots for four RI/MHW groups.
#
# Outputs:
# - A multi-panel PDF figure: 'Surge_Tp_cost.pdf'
#
# Disclaimer:
...
for i, (ax, (dataset, cmap), dataset_label, shade_color) in enumerate(zip(axes.flatten(), datasets, dataset_labels, shaded_colors)):
    ...
    #ax.set_title(label)
    ax.set_xlabel('Mean Tp over land [mm/hr]')
    ...
    # ax.fill_between(x=[1.5, ax.get_xlim()[1]], y1=0, y2=ax.get_ylim()[1], color=shade_color, alpha=0.1)
    ...
    scatter_other = ax.scatter(
        ...
        dataset[dataset['Source'] != 'EM-DAT']['max_surge'], 
...
plt.savefig('.../Surge_Tp_cost.pdf', format='pdf', bbox_inches='tight')
