...
#
# This script analyzes the total number of rapid intensification (RI) tropical cyclones 
# over the Southwest Pacific basin from 1981 to 2023. It compares the yearly frequency of 
# RI vs. non-RI storms and fits OLS regression trend lines to evaluate decadal trends.
#
# Outputs:
# - Bar plot showing RI and non-RI counts per year
# - Trend lines with 90% confidence intervals for RI and total TCs
# - Percentage changes annotated across multi-year intervals
#
...
basins = {
    ...
    'Southwest Pacific': {'range': (-40, 0, -180, -140), 'color': '#CAB767'}
}

min_lat, max_lat, min_lon, max_lon = (-40, 0, -180, -140)
...
data_mhw_ri = data_mhw_ri[(data_mhw_ri['HI_LAT'] >= min_lat) & (data_mhw_ri['HI_LAT'] <= max_lat) &
                            (data_mhw_ri['HI_LON'] >= min_lon) & (data_mhw_ri['HI_LON'] <= max_lon)]

# Filter for NA basin
data_all_ri = data_all_ri[(data_all_ri['HI_LAT'] >= min_lat) & (data_all_ri['HI_LAT'] <= max_lat) &
                            (data_all_ri['HI_LON'] >= min_lon) & (data_all_ri['HI_LON'] <= max_lon)]

data_ibtracs['ISO_TIME'] = pd.to_datetime(data_ibtracs['ISO_TIME'], errors='coerce')
data_ibtracs['Year'] = data_ibtracs['ISO_TIME'].dt.year
data_ibtracs = data_ibtracs[(data_ibtracs['LAT'] >= min_lat) & (data_ibtracs['LAT'] <= max_lat) &
                          (data_ibtracs['LON'] >= min_lon) & (data_ibtracs['LON'] <= max_lon)]
...
# Calculate counts
total_tcs_per_year = data_ibtracs.groupby('SEASON')['NAME'].nunique().reindex(years_range, fill_value=0)
ri_tcs_per_year = data_all_ri.groupby('SEASON')['NAME'].nunique().reindex(years_range, fill_value=0)

# Calculate non-RI TCs per year
tc_without_ri_per_year = total_tcs_per_year - ri_tcs_per_year

# Proportion calculations
proportion_ri_counts = ri_tcs_per_year
proportion_non_ri_counts = tc_without_ri_per_year
...
proportion_ri_counts.plot(kind='bar', ax=ax, color='#CAB767', label='RI TCs')
proportion_non_ri_counts.plot(kind='bar', ax=ax, color='lightgray', label='Non-RI TCs', bottom=proportion_ri_counts)

# Plot the trend line for RI counts
ax.plot(np.arange(len(proportion_ri_counts)), fitted_values_ri, 'r-', label='RI TCs Trend')
...
ax.plot(np.arange(len(proportion_ri_counts)), fitted_values_combined, 'k-', label='All TCs Trend')
...
if results_ri.pvalues[1] <= 0.1:
    trend_label_ri = 'RI TCs Trend**'
else:
    trend_label_ri = 'RI TCs Trend'

if results_combined.pvalues[1] <= 0.1:
    trend_label_combined = 'All TCs Trend**'
else:
    trend_label_combined = 'All TCs Trend'
...
ax.set_title('Southwest Pacific', fontsize=14)

# Customize the legend to include only the specified items in the desired order
handles, labels = ax.get_legend_handles_labels()
desired_labels = ['RI TCs', 'Non-RI TCs', trend_label_ri, trend_label_combined]
desired_handles = [handles[labels.index(label)] for label in ['RI TCs', 'Non-RI TCs']]

# Add trend handles separately to avoid index issues
desired_handles.append(handles[labels.index('RI TCs Trend')])
desired_handles.append(handles[labels.index('All TCs Trend')])
...
for i, period in enumerate(sorted(averages_for_periods.keys())):
    ...
    if i > 0:
        ...
        text_y = max(avg_value, previous_avg) + 0.6  # Midpoint on the y-axis between the two averages
...
plt.tight_layout()
plt.savefig('.../ri_total_SPac.pdf', format='pdf')
plt.show()
