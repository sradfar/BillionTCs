...
#
# This script visualizes joint exceedance probabilities of wind speed and precipitation 
# and their relationship to tropical cyclone economic damages across four RI/MHW groups. 
# It includes copula-based shaded plots and count breakdowns by cost tier.
#
# Outputs:
# - A multi-panel PDF figure: 'Tp_Ws_cost.pdf'
#
# Disclaimer:
...
# RI_with_MHW_df = pd.read_csv('.../Nature data/RI_with_MHW_filled.csv')
# RI_without_MHW_df = pd.read_csv('.../Nature data/RI_without_MHW_filled.csv')
# without_RI_df = pd.read_csv('.../Nature data/without_RI_filled_Land1.csv')
# without_RI_MHW_df = without_RI_df[without_RI_df['MHW_land'] == 1]
# without_RI_no_MHW_df = without_RI_df[without_RI_df['MHW_land'] != 1]
...
# datasets = [(filtered_RI_MHW, 'RdGy_r'), (filtered_RI_noMHW, 'RdBu_r'),
#             (filtered_noRI_MHW, 'BrBG_r'), (filtered_noRI_noMHW, 'PuOr_r')]
...
for i, (ax, (dataset, cmap), dataset_label, shade_color) in enumerate(zip(axes.flatten(), datasets, dataset_labels, shaded_colors)):
    ...
    ax.set_ylabel('Mean Tp over land [mm/hr]')
    ax.set_xlim(10, 160)
    ax.set_ylim(0, 3)
...
plt.savefig('.../Tp_Ws_cost.pdf', format='pdf', bbox_inches='tight')
